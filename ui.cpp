...
// Paint the "instrument paper" background: a flat warm paper fill overlaid with
// a faint graph-paper grid (fine cells + heavier 5-cell rule), mirroring the
// body background on detectmyjiggler.com.
void DrawGradientBackground(HWND hwnd, HDC hdc) {
    ...
    const int fine = 24;    // fine grid spacing (px)
    const int rule = fine * 5; // heavier rule every 5 cells

    // Fine grid — very subtle warm hairlines.
    HPEN finePen = CreatePen(PS_SOLID, 1, RGB(225, 223, 213)); // ~ ink @ very low alpha on paper
    HPEN oldPen = (HPEN)SelectObject(hdc, finePen);
    for (int x = rc.left; x < rc.right; x += fine) {
        MoveToEx(hdc, x, rc.top, nullptr);
        LineTo(hdc, x, rc.bottom);
    }
    for (int y = rc.top; y < rc.bottom; y += fine) {
    ...
    // Heavier 5-cell rule.
    HPEN rulePen = CreatePen(PS_SOLID, 1, RGB(216, 214, 204));
    oldPen = (HPEN)SelectObject(hdc, rulePen);
    for (int x = rc.left; x < rc.right; x += rule) {
        MoveToEx(hdc, x, rc.top, nullptr);
        LineTo(hdc, x, rc.bottom);
    }
    for (int y = rc.top; y < rc.bottom; y += rule) {
        MoveToEx(hdc, rc.left, y, nullptr);
        LineTo(hdc, rc.right, y);
    }
    SelectObject(hdc, oldPen);
...
void ApplyListViewTheme(HWND listView) {
    if (!listView) return;
    // The device list reads as a data panel: lighter surface, ink text.
    ListView_SetBkColor(listView, gSurfaceColor);
    ListView_SetTextBkColor(listView, gSurfaceColor);
    ListView_SetTextColor(listView, gInkColor);

    HWND header = (HWND)SendMessage(listView, LVM_GETHEADER, 0, 0);
    if (header) {
        SetWindowTheme(header, L"Explorer", nullptr);
        SendMessage(header, WM_SETFONT, (WPARAM)hFontMono, TRUE);
    }
}
    ...
    switch (uMsg) {
        case WM_CREATE: {
            ...
            hReasonFont = CreateFont(-13, 0, 0, 0, FW_SEMIBOLD, FALSE, FALSE, FALSE,
                DEFAULT_CHARSET, OUT_OUTLINE_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY,
                VARIABLE_PITCH, "Segoe UI");
            hPopupBgBrush = CreateSolidBrush(gSurfaceColor);
        ...
        case WM_CTLCOLORSTATIC: {
            ...
            if (ctrlId == CUSTOM_POPUP_TITLE_ID) { // Title
                SetTextColor(hdc, gFlagColor); // Rust — flagged
            } else if (ctrlId == CUSTOM_POPUP_ICON_ID) { // Icon
                SetTextColor(hdc, gFlagColor);
            } else if (ctrlId == CUSTOM_POPUP_REASON_ID) { // Detection reason
                SetTextColor(hdc, gFlagColor); // Rust highlights the reason
            } else {
...
int ShowCustomJigglerPopup(HWND parent) {
    ...
    if (!registered) {
        ...
        wc.hbrBackground = CreateSolidBrush(gSurfaceColor); // single flat surface — no two-tone vs. control backgrounds
        RegisterClass(&wc);
        registered = true;
    }
    ...
    switch (uMsg) {
        case WM_CREATE: {
            ...
            hTextFont = CreateFont(-14, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
                DEFAULT_CHARSET, OUT_OUTLINE_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY,
                VARIABLE_PITCH, "Segoe UI");
            hTpBgBrush = CreateSolidBrush(gSurfaceColor);
    ...
    switch (uMsg) {
        case WM_CREATE: {
            ...
            hTextFont = CreateFont(-14, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
                DEFAULT_CHARSET, OUT_OUTLINE_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY,
                VARIABLE_PITCH, "Segoe UI");
            hCalBgBrush = CreateSolidBrush(gSurfaceColor);
        ...
        case WM_CTLCOLORSTATIC: {
            ...
            SetBkMode(hdc, TRANSPARENT);
            if (ctrlId == CUSTOM_POPUP_TITLE_ID) {
                SetTextColor(hdc, gSignalColor); // Green — start / positive
